...
export default function ManageFolderTasksPage({ folder, folderId, onBack }) {
  ...
  const dataObj = useMemo(() => ensureFoldersData(loadCachedData()), [folderId]);
  ...
  function TaskRow({ t }) {
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 14, alignItems: "center" }}>
        <div style={{ padding: "12px 14px", borderRadius: 12, border: "2px solid rgba(255,165,0,0.95)", background: "rgba(245,228,178,0.95)", color: "rgba(20,12,8,0.95)", fontWeight: 1000, textTransform: "lowercase" }}>
          {t.task_name || "task"}
        </div>
        <div style={{ padding: "10px 12px", borderRadius: 12, border: "2px solid rgba(255,165,0,0.95)", background: "rgba(20,12,8,0.60)", display: "flex", gap: 10, alignItems: "center", overflowX: "auto" }}>
          {Array.from({ length: cost }).map((_, i) => {
            const isDone = i < done;
            return (
              <button key={`${t.id}_s${i}`} type="button" onClick={() => toggleSpoon(t.id, i)} title={isDone ? "Undo to here" : "Complete to here"} style={{ width: 40, height: 32, borderRadius: 8, border: "2px solid rgba(255,165,0,0.95)", background: isDone ? "rgba(240,220,150,0.95)" : "rgba(0,0,0,0.55)", color: "rgba(20,12,8,0.95)", fontWeight: 1000, cursor: "pointer", flex: "0 0 auto" }}>
                {isDone ? "🥄" : ""}
              </button>
            );
          })}
